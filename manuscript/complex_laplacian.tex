...
\begin{equation}
\label{eq3}
j\omega \bar{X}(\omega) = -\beta (I - \alpha C(\omega)) \bar{X}(\omega)
\end{equation}

By factoring out the diffusion constant $\beta$ from both terms, and having $\alpha$ act solely as the global coupling parameter, we obtain the expression $\mathcal{L} = (\omega) = I - \alpha C(\omega)$, where $I$ is the identity matrix and $\mathcal{L}(\omega)$ is the complex Laplacian matrix.
